...
class vgg16:
    ...
    def convlayers(self, n_classes, f):
        ...
        with tf.name_scope('fc6') as scope:
            kernel = tf.Variable(tf.truncated_normal([1, 1, f[3], f[4]], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
            conv = tf.nn.conv2d(self.pool4, kernel, [
                                1, 1, 1, 1], padding='SAME')
            biases = tf.Variable(tf.constant(0.0, shape=[f[4]], dtype=tf.float32),
        ...
        with tf.name_scope('upscore') as scope:
            kernel = tf.Variable(tf.truncated_normal([64, 64, n_classes, f[4]], dtype=tf.float32,  # 2 here is N output classes
                                                     stddev=1e-1), name='weights')
