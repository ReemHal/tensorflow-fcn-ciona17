...
def _rec_separate_layers(combined_layers, times_combined, counter=0):
    unpacked = _elegant_unpair(combined_layers)
    layers = []
...
# The unpairing for the Elegant Pairin function
def _elegant_unpair(z):
    sqrtz = math.floor(math.sqrt(z))
    if ((sqrtz*sqrtz) > z): # accounts for a rounding error of 1 for large numbers
        sqrtz = sqrtz - 1
    sqz = sqrtz * sqrtz;
    if ((z - sqz) >= sqrtz):
        x = sqrtz
        ...
        x = z - sqz
        y = sqrtz
    return x, y #((z - sqz) >= sqrtz) ? [sqrtz, z - sqz - sqrtz] : [z - sqz, sqrtz];
